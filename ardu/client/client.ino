...
#define IR_2 3
#define IR_3 4
static bool readIR1(uint32_t arg) {
  return digitalRead(IR_1==LOW);      
  //input_pullup 이라 찼으면 LOW 반환
}
static bool readIR2(uint32_t arg) {
  return digitalRead(IR_2==LOW);
}
static bool readIR3(uint32_t arg) {
  return digitalRead(IR_3==LOW);
}
...
static bool cmdEntryGate(uint32_t arg){
  switch(arg){
    case 1:
      ...
      router.echoIs(true);    // arg=1 은 열린 상태 -> 서버의 Echo 상태도 on
#if DEBUG
...
#define US_NEAR_CM     10      // (60)이보다 가까우면 "차가 있다"
...
#define US2_TRIG 9   // 배선 출처: 사용자 2026-08-20 "트리거가 11 에코가 10 하나 더달았다"
...
void setup() {
  ...
  node.sensor  ("R1").on(readIR1);
  node.sensor  ("R2").on(readIR2);
  node.sensor  ("R3").on(readIR3);
  ...
  Serial.println(F("R1 : Parking Slot 1"));
  Serial.println(F("R2 : Parking Slot 2"));
  Serial.println(F("R3 : Parking Slot 3"));
