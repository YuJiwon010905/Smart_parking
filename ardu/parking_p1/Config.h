#pragma once
// ═════════════════════════════════════════════════════════════════════════
// Config.h — 배선·망 설정 · 타이밍 · 반송파 슬롯
//   🔓 **기여자가 고치는 곳**: 아래 `WIFI_SSID` · `WIFI_PASS` · `SERVER_IP` · `SERVER_PORT`
//   ⚙ 응용 성질. 잠금 대상이 아니다.
//
// 🔴🔴 **이 파일의 위치를 옮기지 마라.** `client.ino` 의 정해진 자리에서 `#include` 된다 —
//   자리나 헤더 순서를 바꾸면 **선언·초기화 순서가 같이 바뀌어 산출물이 달라진다.**
// ═════════════════════════════════════════════════════════════════════════

// ─────────────────────────────────────────────────────────────────────────
// 배선 · 네트워크 상수
// ─────────────────────────────────────────────────────────────────────────
static const uint8_t PIN_ESP_RX = A0;   // ESP TX → Uno
static const uint8_t PIN_ESP_TX = A1;   // Uno → ESP RX
SoftwareSerial wifi(PIN_ESP_RX, PIN_ESP_TX);

#define WIFI_SSID    "3F_302"   // 🔓 자기 공유기 이름으로 바꿔라
// 🔴 **ESP8266 은 2.4GHz 전용이다. 5GHz SSID 를 넣으면 영영 안 붙는다.**
//   ⚠ 이름에 대역 표시(`_2.4G`)가 없는 공유기는 **2.4GHz 쪽 SSID 가 따로 있는지 확인해라.**
#define WIFI_PASS    "0424719222!!"
// ⚠ 비밀번호에 `!` 가 들어 있으면 **셸에서 편집하지 마라** — 히스토리 확장이 값을 바꾼다.
//   **편집기로 직접 넣어라.** 값이 바뀔 때마다 다시 본다.
// ⚠⚠ **앞뒤 공백을 절대 넣지 마라.** 구운 펌웨어에 `" 192.168.35.21"` 로 앞 공백이 들어가 있었고,
//    그래서 ESP 가 IP 리터럴로 못 읽고 **호스트명으로 해석해 `DNS Fail`** 을 냈다.
//    (기기 플래시를 읽어 확인한 실물 문자열: AT+CIPSTART="TCP"," 192.168.35.21",9991)
//    이 매크로는 그대로 AT 명령에 이어붙으므로 공백 하나가 곧 고장이다.
#define SERVER_IP    "192.168.0.82"  // 🔓 서버를 돌리는 기계의 주소. `ipconfig getifaddr en0` 로 확인한다
                                      //   ⚠ 망을 옮기면 서브넷째로 바뀐다. **현장에서 다시 잰다**
// 🔓 **기여자용 기본값은 dev 서버(9991)다. 이 파일에는 이 값 하나만 있다.**
//
// 🔴 **왜 dev 가 기본인가 — 틀렸을 때의 방향이 비대칭이다:**
//   · 9991 인 채로 운영에 쓰면 → **안 붙는다. 즉시 드러난다**
//   · 8888(운영)인 채로 dev 에 쓰면 → 🔴 **운영 서버에 붙는다.** `devid` 가 겹치면
//     **남의 노드를 쫓아내고**, 그런데 **로컬에서는 멀쩡하다** — 합류할 때만 터진다
//   🔑 **틀렸을 때 조용한 쪽을 기본값으로 두지 않는다.**
//
// ⚠ **`SERVER_IP` 와 한 벌이다.** dev 와 운영이 다른 기계면 **둘 다** 바꿔야 한다.
//   "포트만 고치면 된다"로 기억하면 IP 를 안 고치고 헤맨다.
//
// ⚠ `#ifndef` 인 이유: **우리 시험 칩만** 빌드 시점에 덮는다(`-DSERVER_PORT='"8888"'`).
//   그래야 **이 파일이 샘플의 진실 하나만** 들고 있다. 지금 칩이 무엇을 보는지는
//   부팅 `[NET] 대상 …` 줄이 말해 준다 — 소스를 믿지 말고 그 줄을 봐라.
#ifndef SERVER_PORT
#define SERVER_PORT  "9991"
#endif
// 🔴 `DEVICE_ID` 는 **`client.ino` 머리**에 있다. **여기서 다시 정의하지 마라** —
//   `client.ino` 가 이 파일보다 먼저 정의한다.

// ─────────────────────────────────────────────────────────────────────────
// 타이밍 (§3.4, §6.3)
// ─────────────────────────────────────────────────────────────────────────
static const uint16_t HEARTBEAT_MS     = 1000;  // ⚠ 슬롯 구조에서는 안 쓴다(아래 반송파 슬롯을 봐라)
static const uint16_t DEBOUNCE_MS      = 100;   // §3.4 변화 디바운스
static const uint16_t PROMPT_TIMEOUT_MS= 300;   // '>' 프롬프트 대기 상한 ("수백 ms")
static const uint16_t SEND_FAIL_BACKOFF_MS = 500; // 전송 실패 후 재시도 최소 간격
static const uint8_t  SEND_GAP_MS      = 80;    // 연속 CIPSEND 사이 최소 간격 (busy p... 회피)

// ─────────────────────────────────────────────────────────────────────────
// 🔴 반송파 슬롯 — 설계 근거는 `docs/DESIGN-slot-carrier-2026-08-17.md` 에 있다
//
//        슬롯 1200ms
//   ├───────────────┬───────────────┤
//   │  0 ~ 600ms    │  600 ~ 1200ms │
//   │  장치 송신     │  수신 전용     │
//   └───────────────┴───────────────┘
//
// **이벤트가 전송을 만들지 않는다. 전송이 상수이고 이벤트는 화물이다.**
//   옛 구조: 이벤트↑ → 전송↑ → 귀 막힘↑ → ACK 실패↑ → 서버 재전송↑ → (처음으로)
//            ⇒ 양의 되먹임. 임계를 넘으면 ESP 리셋까지 간다.
//   새 구조: 이벤트↑ → 전송 횟수 그대로 → **한 프레임의 크기만↑**
//            ⇒ 구조적으로 커질 수 없다.
//
// 🔑 **수신 창에는 한 바이트도 쓰지 않는다.** 이것이 손실 0 의 직접 원인이다
//   (fdtest 1판 `gap=36 bad=32` → 2판 `gap=0 bad=0`).
//   `SoftwareSerial` 은 송신 중 `cli()` 로 인터럽트를 끄고 수신은 그 인터럽트로만 받는다 —
//   **겹치는 순간 우리는 귀가 닫힌다.** 슬롯은 그 겹침 자체를 없앤다.
//
// ⚠ 슬롯 시작은 **시계가 아니라 사건**이다 — 장치의 프레임 도착이 서버의 t0 다.
//   그래서 시계 동기가 필요 없고, 장치가 느려지면 서버 슬롯도 같이 밀린다(자기교정).
static const uint16_t SLOT_MS          = 1200;  // 한 슬롯
static const uint16_t TX_WINDOW_MS     = 600;   // 0~600 우리 차례 · 600~1200 수신 전용
// 🔴 `[CNT]` 진단 줄이 송신 창을 넘지 않도록 남겨 두는 시간
//   `[CNT]` 약 142B @115200 = 12.3ms 이고, TX 링버퍼(64B)를 넘는 78B 가 블로킹으로 나간다.
//   `[RAM]` 이 뒤따르는 경우까지 덮도록 여유를 준다. **송신 창 600ms 대비 8% 라 배치를 안 밀어낸다.**
static const uint16_t SLOT_TX_RESERVE_CNT_MS = 50;
// 배치 버퍼 — **한 거래에 여러 줄**을 담는다. 줄 하나의 64B 상한(§2.1)은 그대로다.
//   최대 = S 프레임 63 + LF + ACKQ_N × (ACK 최대 ~20 + LF)
//   ⚠ `AT+CIPSEND` 는 300B 까지 조용한 잘림이 없음이 실측 확인됐다(커밋 `1299286`, 18/18).
static const uint8_t  BATCH_CAP        = 160;

#include "Slots.h"   // 자리·센서. **이 자리에서 include 된다 — 옮기지 마라**

