...
static void processCommand(char* cand) {
  ...
  if (idx == 0xFF) {
    ...
    // CODEX FIX: reservation/business validation belongs to the server.
    // Arduino only mirrors the authoritative R/C command into its local mask
    // and acknowledges whether the frame could be applied.
    if (type == 'R') {
      node.resMask |= bit;
      result = 0;
    } else {                                            // 'C' — 취소
