// wire.h — 송신 helper · WebSocket 프레임 (§5.2). `struct Server` 의 몸통 조각.
    ...
    void send_node(Node& node, const std::string& line) {
        ...
        logf("→ARD", node.devid + " " + (line.empty() ? std::string() : line.substr(0, line.size() - 1)));
    }
    ...
    void flush_downq(Node& node, const char* why, bool ignore_window, int max_n = DOWN_BATCH_MAX_N) {
        ...
        for (size_t i = 0; i < batch.size(); i++) {
            ...
            char ph[64];
            if (!node.seen) snprintf(ph, sizeof(ph), " (%s S미수신)", node.devid.c_str());
            else            snprintf(ph, sizeof(ph), " (%s S+%lldms)", node.devid.c_str(), (long long)(t - node.last_ms));
