// nodes.h — 다중 노드 — 등록·승격·결속·라우팅 (REQ-0083). `struct Server` 의 몸통 조각.
    ...
    void adopt_as_parking(sock_t c, const std::string& dev) {
        ...
        reboot_by_conn++;
        park.session_reset();      // MULTI: 등록·상태·창을 node별 초기화
        ard_peer = peer_str(c);
    ...
    void adopt_as_aux(sock_t c, const std::string& dev) {
        ...
        if (it != aux.end() && it->second.fd != BAD_SOCK) {
            ...
            if (lot.unbindDevice(dev)) bump_epoch("node reconnect binding 초기화 " + dev);
        ...
        a.fd = c; a.buf.clear(); a.session_reset(); a.devid = dev;
    ...
    void reap_nodes() {
        ...
        for (size_t k = 0; k < reap.size(); k++) {
            ...
            clear_downq_for(reap[k], "node 유휴 마감");
            if (lot.unbindDevice(reap[k])) bump_epoch("node disconnect binding 해제 " + reap[k]);
    ...
    // MULTI: node-local module index를 Lot binding을 통해 global A1~A5로 변환한다.
    void sync_parking_slots_from_nodes() {
        bool any_test = false;
        for (size_t zi = 0; zi < lot.zones().size(); zi++) {
            ...
            if (total == 0 || known != total) continue; // unknown을 EMPTY로 무너뜨리지 않는다
            ...
            if (before && !after && slots[si].reserved && !has_pending_for(z.id)) retire(si);
        }
        test_armed = any_test;
    }
    ...
    void notify_occupancy_changes() {
        for (size_t i = 0; i < lot.zones().size(); i++) {
            ...
            // CODEX FIX: entrance/exit are sensor-bearing general areas.
            // Report every I* module to the server controller; only parking
            ...
            for (size_t m = 0; m < ms.size(); m++) {
                ...
                if (first) {
                    // CODEX FIX: the controller needs an initial snapshot before
                    // it can safely choose a free slot. Publish the first known
                    // value instead of waiting for an edge that may never occur.
                    occ_prev_[key] = now;
                    it = occ_prev_.find(key);
                } else {
                    if (it->second == now) { occ_fall_n_[key] = 0; continue; }
                    // 하강(찼다→비었다)만 한 프레임 더 확인한다.
                    if (!now && ++occ_fall_n_[key] < 2) continue;
                    occ_fall_n_[key] = 0;
                    it->second = now;
                }
    ...
    void node_send_failed(Node& n, const std::string& why) {
        ...
        n.fd = BAD_SOCK; n.online = false; n.mod_bits_n = 0;
        n.res_bits_n = 0; n.test_bits_n = 0;
        clear_downq_for(dev, why.c_str());
        if (lot.unbindDevice(dev)) bump_epoch("node disconnect binding 해제 " + dev);
    ...
    int decode_mod_bits(const std::string& fld, int* out) const {
        return decode_mod_bits(park, fld, out); // 기존 selftest/단일 node 호환
    }
    ...
    int reg_cmdable() const { return reg_cmdable(park); }
