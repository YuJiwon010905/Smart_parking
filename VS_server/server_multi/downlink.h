// downlink.h — 아두이노 하행(서버 → 장치) 발행·재전송·회수. `struct Server` 의 몸통 조각.
    ...
    void notify_cmd(const Pending& p, CmdResult::Kind k, int devResult) {
        ...
        CmdResult r;
        r.kind = k; r.devid = p.devid; r.module = p.slot;
    ...
    bool send_to_module(const std::string& devid, const std::string& name, long arg,
        ...
        if (!n->reg_done || !node_online(*n)) {
            logf("!", "발행 실패 — 노드 `" + devid + "` 가 offline 또는 등록 미완료");
            return false;
        }
        int idx = -1;
    ...
    void tick() {
        ...
        std::vector<Node*> tick_nodes = all_nodes();
        for (size_t ni = 0; ni < tick_nodes.size(); ni++) {
            Node& qn = *tick_nodes[ni];
            if (qn.fd == BAD_SOCK || !qn.seen || qn.reg_done || qn.reg_giveup
                || !qn.reg_first_ms || (t - qn.reg_first_ms) <= REG_TIMEOUT_MS
                || (t - qn.last_q_ms) < DOWN_SLOT_MS) continue;
            if (qn.q_sent >= REG_Q_MAX) {
                ...
                if (!send_raw(qn.fd, q.data(), q.size(), qn.devid.c_str())) node_send_failed(qn, "Q 송신 실패");
        ...
        for (size_t ni = 0; ni < tick_nodes.size(); ni++) {
            Node& dn = *tick_nodes[ni];
            bool has_queued = false;
            for (size_t qi = 0; qi < downq.size(); qi++)
                if (downq[qi].devid == dn.devid) { has_queued = true; break; }
            if (has_queued && dn.fd != BAD_SOCK && (t - dn.last_ms) > DOWN_DMAX_MS
                && dn.dmax_armed && (t - dn.last_dmax_ms) >= DOWN_SLOT_MS) {
                dn.last_dmax_ms = t;
                dn.dmax_armed = false;
                dmax_flushes++;
