...
struct Node {
    ...
    // MULTI: S frame과 송신 창은 socket/Arduino별 상태다.
    long        seq;
    long long   uptime;
    int         res_bits[REG_MODS_MAX];
    int         res_bits_n;
    int         test_bits[REG_MODS_MAX];
    int         test_bits_n;
    bool        test_armed;
    ...
    void session_reset() {
        ...
        mod_bits_n = res_bits_n = test_bits_n = 0;
        test_armed = false; last_dmax_ms = 0; dmax_armed = true;
        reg_reset();
    }
};
