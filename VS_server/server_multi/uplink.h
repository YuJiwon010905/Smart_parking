// uplink.h — 아두이노 상행(장치 → 서버) 파서. `struct Server` 의 몸통 조각.
    ...
    void on_ard_line(Node& n, const std::string& line) {
        ...
        if (f[0] == "S" && f.size() >= 6) {
            ...
            if (f[5] != n.devid) {
                logf("!!", "socket node와 S devid 불일치 — expected=" + n.devid + " actual=" + f[5]);
                return;
            }
            n.seq = seq; n.uptime = up;
            n.last_ms = now_ms(); n.last_epoch_ms = epoch_ms(); n.seen = true; n.online = true;
            ...
            if (f.size() >= 7 && f[6] != "-" && !f[6].empty()) {
                ...
                armed = true;
                if (decode_mod_bits(n, f[6], ovr_bits_) > 0)
            ...
            // MULTI: module index는 node-local이다. global slot 1→0 판정은
            // sync_parking_slots_from_nodes()가 Zone binding으로 수행한다.
            n.test_armed = armed;
            n.test_bits_n = armed ? n.mod_bits_n : 0;
            for (int i = 0; i < REG_MODS_MAX; i++) n.test_bits[i] = (i < 10 ? ovr[i] : 0);
            if (test_armed != armed)
            ...
            n.res_bits_n = res_n_ > 0 ? res_n_ : 0;
            if (res_n_ > 0)
                for (int i = 0; i < REG_MODS_MAX; i++) n.res_bits[i] = res_bits_[i];
            if (res_n_ <= 0) {
                res_undecoded++;
                ...
                for (size_t mi = 0; mi < n.mods.size() && mi < (size_t)res_n_; mi++) {
                    const std::string zid = lot.zoneOfModule(n.devid, n.mods[mi].first, lot_);
                    const int i = slot_index(zid);
                    if (i >= 0 && res_bits_[mi] && slots[i].reserved == 0 && !has_pending_for(SLOT_ID[i])) {
                        heal_fires++;
        ...
        //   **세션마다 11씩 오르며 거짓 경보를 만든다.**
            ...
            if (f[1] == "*") {
                ...
                n.prev_mods_snapshot = n.mods;   // MULTI: node별 재등록 비교
                n.reg_reset();
            ...
            if ((int)n.mods.size() == n.reg_n) {
                ...
                resync_reservations("node 등록 완료");
            ...
            Pending p = it->second;
            if (p.devid != n.devid) {
                logf("!!", "ACK device 불일치 rid=" + std::to_string(rid)
                           + " expected=" + p.devid + " actual=" + n.devid);
                return;
            }
            pend.erase(it);
            rid_release(rid);
