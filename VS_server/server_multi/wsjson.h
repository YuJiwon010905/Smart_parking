// wsjson.h — 봉투 만들기 — map/state/snapshot JSON. `struct Server` 의 몸통 조각.
    ...
    void emit_module_cmd(std::ostringstream& o, const std::string& devid,
        ...
        else if (!mn || !node_online(*mn)) blk = "device_offline";
    ...
    std::string snapshot_json() {
        ...
        o << "}";
        // MULTI: 기존 device(P1 호환)는 유지하고 모든 Arduino를 별도 배열로 제공한다.
        o << ",\"devices\":[";
        {
            std::vector<Node*> ns = all_nodes();
            for (size_t i = 0; i < ns.size(); i++) {
                if (i) o << ",";
                Node& n = *ns[i];
                o << "{\"device_id\":" << jstr(n.devid)
                  << ",\"online\":" << (node_online(n) ? "true" : "false")
                  << ",\"uptime\":" << (n.uptime < 0 ? 0 : n.uptime)
                  << ",\"seq\":" << (n.seq < 0 ? 0 : n.seq)
                  << ",\"last_frame_ts\":";
                if (n.last_epoch_ms > 0) o << n.last_epoch_ms; else o << "null";
                o << "}";
            }
        }
        o << "]";
