    ...
    void write_log_if_changed() {
        ...
        key += online_now ? "|O" : "|X";
        {
            ...
            for (size_t i = 0; i < ns.size(); i++)
                key += "|" + ns[i]->devid + (node_online(*ns[i]) ? ":O" : ":X");
        }
        ...
        e << ",\"devices\":[";
        {
            ...
            for (size_t i = 0; i < ns.size(); i++) {
                if (i) e << ",";
                Node& n = *ns[i];
                e << "{\"device_id\":" << jstr(n.devid)
                  << ",\"online\":" << (node_online(n) ? "true" : "false")
                  << ",\"uptime\":" << (n.uptime < 0 ? 0 : n.uptime)
                  << ",\"seq\":" << (n.seq < 0 ? 0 : n.seq)
                  << ",\"last_frame_ts\":"
                  << (n.last_epoch_ms > 0 ? std::to_string(n.last_epoch_ms) : std::string("null"))
                  << "}";
            }
        }
        e << "]"
        ...
        for (size_t zi = 0; zi < lot.zones().size(); zi++) {
            ...
            const int i = slot_index(z.id);
            if (i < 0) continue;
            if (!first_slot) e << ",";
            first_slot = false;
            e << "{\"id\":" << jstr(z.id) << ",\"occupied\":" << slots[i].occupied
